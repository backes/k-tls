...
static mod_mem_info_t *mod_mem_init_tx(TXPARAM);
...
#ifdef DEBUG2
  TX_GET;
  PRINT_DEBUG2("==> stm_malloc(tx=%p,size=%lu)\n", tx, size);
#endif
...
/*
 * Called by the CURRENT thread to free memory within a transaction without taking any locks.
 */
void stm_free0(TXPARAMS void *addr)
{
  stm_free2(TXARGS addr, 0, 0);
}
...
{
  PRINT_DEBUG2("==> stm_free2(addr=%p,idx=%lu,size=%lu)\n", addr, idx, size);
...
# endif
  PRINT_DEBUG2("==> mod_mem_init_tx(%p)\n", tx);
#endif
...
# ifndef EXPLICIT_TX_PARAMETER
  stm_tx_t *tx = stm_get_tx()
# endif
  PRINT_DEBUG2("==> mod_mem_on_commit(%p)\n", tx);
#endif
  ...
  free(mi);
  stm_set_specific(TXARGS mod_mem_key, 0);
}
...
# ifndef EXPLICIT_TX_PARAMETER
  stm_tx_t *tx = stm_get_tx()
# endif
  PRINT_DEBUG2("==> mod_mem_on_abort(%p)\n", tx);
#endif
...
void mod_mem_init(int gc)
{
  ...
  stm_register(NULL, NULL, NULL, NULL, mod_mem_on_commit, mod_mem_on_abort, NULL);
