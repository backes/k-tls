...
#ifndef INLINE
# define INLINE(STR) inline STR __attribute__((always_inline))
#endif
#ifndef NORETURN
# define NORETURN(STR) STR __attribute__((noreturn))
#endif
...
/**
 * Initialize a transactional thread.  This function can optionally be
 * called once from each thread that performs transactional operations,
 * before the thread calls any other functions of the library.
 */
 ...
 * @return
 *   1 upon success
 *   0 is never returned, as in this case the transaction is
 *   automatically rolled back and reexecuted
 */
 ...
 */
NORETURN(void) stm_abort(TXPARAMS int abort_reason);
 ...
 * directly after starting a transaction.
 * If the current thread is already executing a
...
/*
 * For the counted time in read/write/commit, only
 * successfull transactions count. On a rollback, all the time
 * (including time in read/write) counts as in_rollbacked_code.
 */
typedef struct tx_stats_per_tx {
  uint64_t nanos_in_read;
  uint64_t nanos_in_write;
} tx_stats_per_tx_t;
typedef struct tx_stats {
  tx_stats_per_tx_t per_tx;

  uint64_t nanos_in_committed_tx; // realtime between start and commit
  uint64_t nanos_in_incomplete_tx; // realtime between start and program exit
  uint64_t nanos_in_init;
  uint64_t nanos_in_commit;
  uint64_t nanos_in_validate;
  uint64_t nanos_in_rollback;
  uint64_t nanos_in_rollbacked_code; // realtime between start and rollback

  // total number of transactions: num_commits + num_rollbacks + num_incomplete
  uint64_t num_commits;
  uint64_t num_rollbacks;
  uint64_t num_incomplete;
  uint64_t sum_readset;
  uint64_t sum_writeset;
} tx_stats_t;

/**
 * Return the timings collected since program startup (or last reset).
 * This returns a struct with zeros if COLLECT_TX_STATS is not set.
 */
